...
<nav class="w-full hidden md:flex fle-row justify-center items-center border-b border-gray-200 px-20 relative" role="menu">
    <div class="w-full max-w-[1920px] relative z-1">
        <button class="nav-scroll-btn nav-scroll-btn--left" id="nav-scroll-left" hidden aria-label="Sola kaydır">
            <i class="icon icon-chevron-left"></i>
        </button>
        <button class="nav-scroll-btn nav-scroll-btn--right" id="nav-scroll-right" hidden aria-label="Sağa kaydır">
            <i class="icon icon-chevron-right"></i>
        </button>
        <ul id="main-nav-list" class="w-full flex flex-row justify-start relative z-100 bg-white gap-4 overflow-y-hidden m-0 p-0 list-none">
            <?php if ($menu_items) : ?>
                <?php foreach ($menu_items as $menu_item) : ?>
                    <?php if ($menu_item->menu_item_parent) continue; ?>
                    <li class="shrink-0 m-0 p-0">
                        <a href="<?php echo esc_url($menu_item->url); ?>" data-item-id="<?php echo esc_attr($menu_item->ID); ?>" class="main-menu-item flex flex-row px-20 py-15 gap-6 text-md cursor-pointer relative whitespace-nowrap">
